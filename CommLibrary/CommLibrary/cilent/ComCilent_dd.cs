...
namespace CommLibrary
{
    public class ComCilent : Cilent
    {
        ...
        {
            if (_comport == null || !_comport.IsOpen || stopping) return;
        ...
        private void onEndRead(IAsyncResult ar)
        {
            //try
            // {
            if (ar == currentAynchResult && !stopping)
            {

                 SerialPort remote = (SerialPort)ar.AsyncState;             
                 if (remote != null)
                {
                    int byteRead = remote.BaseStream.EndRead(ar);                   
                    if (byteRead > 0)
                    {
